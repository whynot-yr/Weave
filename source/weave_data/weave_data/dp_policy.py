...
class DPReferencePredictor:
    ...
    def __init__(self, checkpoint, device="cuda"):
        ...
        expected = {"observation.state": (88,), IMAGE_KEY: (3, 224, 224)}
        if {k: tuple(v.shape) for k, v in config.input_features.items()} != expected:
            raise ValueError("Checkpoint input features do not match WEAVE RGB/state protocol")
        if {k: tuple(v.shape) for k, v in config.output_features.items()} != {"action": (125,)}:
            raise ValueError("Checkpoint output features do not match WEAVE reference125")
    ...
    def predict_encoded(self, rgb, state):
        """RGB uint8[B,224,224,3], encoded float state[B,88]; CPU physical-unit output."""
        if rgb.dtype != torch.uint8 or rgb.ndim != 4 or tuple(rgb.shape[1:]) != (224, 224, 3):
            raise ValueError("Expected uint8 RGB [B,224,224,3]")
        if state.shape != (len(rgb), 88) or not torch.isfinite(state).all():
            raise ValueError("Expected finite encoded state [B,88]")
        batch = {"observation.state": state.float()[:, None], IMAGE_KEY: rgb.permute(0, 3, 1, 2).float()[:, None] / 255}
        self.policy.reset()
        action = self.post(self.policy.predict_action_chunk(self.pre(batch)))
        if action.shape != (len(rgb), 40, 125) or not torch.isfinite(action).all():
            raise RuntimeError("Invalid DP prediction")
        return action.cpu()
    ...
    def predict(self, rgb, pelvis_quat, joint_pos, joint_vel, initial_pelvis_quat):
        ...
        return self.predict_encoded(rgb, state)
